                    ...
                    bool                updated)
{
	if (flag & NBIOT_SET_RESID )
	{
		...
		if (!updated || (data->flag&NBIOT_UPDATED))
		{
			...
			{
				sprintf((char *)array,"%.3f",data->value.as_float);
				len = strlen((char *)array);
				value = array;
			}
                     ...
                     nbiot_write_callback_t write_func)
{
    ...
    if (uri->flag & NBIOT_SET_RESID)
    {
		...
		StrToHex(temp_buf, (char *)(buffer), buffer_len);
        ...
        if (data->type == NBIOT_BOOLEAN)
        {
            data->value.as_bool = nbiot_atoi((char *)buffer,2);
        }
        ...
        if (data->type == NBIOT_INTEGER)
        {
			...
			{
				data->value.as_int += temp_buf[i] * pow(10,i);
			}
			
//            data->value.as_int = nbiot_atoi((char *)temp_buf,buffer_len / 2);
        }
        if (data->type == NBIOT_FLOAT)
        {
			for(i = 0; i < 8; i ++)
			{
				convert_buf[i] = temp_buf[7 - i];
			}
			
			memcpy(&(data->value.as_float),convert_buf,buffer_len);
...
//            data->value.as_float = atof((char *)buffer);
            ...
            data->type == NBIOT_BINARY)
        {
            ...
            data->value.as_buf.val = nbiot_strdup((char*)temp_buf, buffer_len);
