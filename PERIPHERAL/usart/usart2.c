...
void USART2_IRQHandler(void)
{
	...
	else if(USART2->SR & USART_FLAG_IDLE)
	{
		...
		if((msg_p = (uint8_t *)strstr((const char *)ring_fifo.data, "+MIPL")) != NULL&&(msg_p = (uint8_t *)strstr((const char *)ring_fifo.data, "+MIPLCREATE")) == NULL) 
		{
			fifo_put(dl_buf_id,ringbuf_elements(&ring_fifo),ring_fifo.data);
		}
		else if((msg_p = (uint8_t *)strstr((const char *)ring_fifo.data, "+CEREG:0")) != NULL)
		{
//			RUN_LED = 1;
		}
